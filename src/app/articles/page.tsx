...
import { getAllArticleMetadata, getCategories, searchArticles, getArticlesByCategory } from '@/lib/articles'
...
export default function ArticlesPage(): React.JSX.Element {
  ...
  const [categories, setCategories] = useState<Array<{ id: string; name: string; color: string; count: number }>>([])
  ...
  useEffect(() => {
    ...
    const formattedCategories = [
      { id: 'all', name: '全て', color: 'gray', count: articles.length },
      ...categoryData.map(cat => ({
        id: cat.slug,
        name: cat.name,
        color: cat.slug === 'fuzoku' ? 'red' : cat.slug === 'fanza' ? 'purple' : 'blue',
        count: cat.count
      }))
    ]
