...
export default function Sidebar({ className = '' }: SidebarProps): React.JSX.Element {
  ...
  return (
    <aside className={`space-y-6 ${className}`} style={{ position: 'sticky', top: '100px' }}>
      {/* 検索ウィジェット */}
      ...
      <div className="content-card">
        ...
        <ul className="space-y-4">
          {recentPosts.map((post) => (
            <li key={post.id} className="pb-3 border-b last:border-b-0"
                style={{ borderColor: 'var(--border)' }}>
              <Link href={`/article/${post.id}`} className="block hover:opacity-80 transition-colors">
