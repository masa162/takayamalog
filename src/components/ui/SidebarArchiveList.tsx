...
export default function SidebarArchiveList({ archivedData }: SidebarArchiveListProps): React.JSX.Element {
  const years = Object.keys(archivedData).sort((a, b) => parseInt(b) - parseInt(a));

  return (
    <ul className="space-y-2">
      {years.map(year => (
        <SidebarArchiveYearItem key={year} year={year} monthsData={archivedData[year]} />
      ))}
    </ul>
  );
}
...
function SidebarArchiveYearItem({ year, monthsData }: SidebarArchiveYearItemProps): React.JSX.Element {
    ...
    <li>
      ...
      {isYearOpen && (
        <ul className="ml-4 mt-1 space-y-1">
          {Object.keys(monthsData).sort((a, b) => parseInt(b) - parseInt(a)).map(month => {
